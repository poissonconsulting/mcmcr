...
The `mcmcr` package also provides the R equivalent to functions such as `pow()`, `logit() <- `
that often occur in JAGS/BUGS, STAN, ADMB/TMB model code.

Finally, the mcmcr package allows the user to `derive` an `mcmcr` object of new parameters (with potentially novel dimensionality) from an existing `mcmcr` object using standard R code.
