...
`mcmcarray` objects were introduced to facilitate manipulation and querying of the MCMC samples (although they are just one `aperm` away from `mcarray` objects they are more intuitive to program with - at least for this programmer!).
`mcmcr` objects were introduced to allow a set of dimensionality preserving parameters to be manipulated and queried as a whole.
