test_that("converged.mcmcr", {
  ...
  expect_identical(converged(mcmcr_example, by = "parameter"),
                   list(alpha = FALSE, beta = FALSE, sigma = TRUE))

  expect_identical(converged(mcmcr_example, by = "parameter", as_df = TRUE),
                   structure(list(parameter = c("alpha", "beta", "sigma"),
                                  converged = c(FALSE, FALSE, TRUE)),
                             row.names = c("alpha", "beta", "sigma"),
                             class = c("tbl_df","tbl", "data.frame")))

  expect_identical(converged(mcmcr_example, by = "term"),
                   list(alpha = c(FALSE, FALSE),
                        beta = structure(c(FALSE, FALSE,
                                           FALSE, FALSE), .Dim = c(2L, 2L)),
                        sigma = TRUE))

  expect_identical(converged(mcmcr_example, as_df = TRUE),
...
test_that("converged.mcmcrs", {
  ...
  expect_identical(converged(mcmcrs, by = "parameter"),
                   list(mcmcr1 = list(alpha = FALSE, beta = FALSE, sigma = TRUE),
                        mcmcr2 = list(alpha = FALSE, beta = FALSE, sigma = TRUE)))
  expect_false(converged(mcmcrs, bound = TRUE), FALSE)
  expect_identical(converged(mcmcrs, bound = TRUE, by = "parameter"),
               list(alpha = FALSE, beta = FALSE, sigma = TRUE))
})
