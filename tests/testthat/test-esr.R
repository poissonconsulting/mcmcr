...
test_that("esr.mcmcr", {
  ...
  expect_identical(esr(mcmcr_example, as_df = TRUE), tibble::tibble(all = "all", esr = 0.011))
  ...
  expect_equivalent(esr(mcmcr_example, by = "term", as_df = TRUE), tibble::tibble(term = as.term(c("alpha[1]", "alpha[2]", "beta[1,1]", "beta[2,1]", "beta[1,2]", "beta[2,2]", "sigma")), esr = c(0.011, 0.011, 0.05, 0.05, 0.05, 0.05, 0.472)))
})
