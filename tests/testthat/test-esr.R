...
test_that("esr.mcmcmarray", {
  ...
  expect_identical(esr(mcmcr_example[[2]], by = "term", as_df = TRUE), tibble::tibble(term = as.term(c("parameter[1,1]", "parameter[2,1]", "parameter[1,2]", "parameter[2,2]")), esr = rep(0.05, 4)))
  expect_identical(esr(mcmcr_example[[3]], by = "term"), c(0.47))
  expect_identical(esr(mcmcr_example[[3]], by = "term", as_df = TRUE), tibble::tibble(term = as.term("parameter"), esr = 0.47))
...
test_that("esr.mcmcr", {
  ...
  expect_identical(esr(mcmcr_example, as_df = TRUE), tibble::tibble(all = "all", esr = 0.01))
  ...
  expect_equivalent(esr(mcmcr_example, by = "term", as_df = TRUE), tibble::tibble(term = as.term(c("alpha[1]", "alpha[2]", "beta[1,1]", "beta[2,1]", "beta[1,2]", "beta[2,2]", "sigma")), esr = c(0.01, 0.01, 0.05, 0.05, 0.05, 0.05, 0.47)))
})
